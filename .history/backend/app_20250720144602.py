...
# JWT config
app.config['JWT_SECRET_KEY'] = '256902ec9f6c83e9cad01fdec8f6567b2b2a38832ec41ccdcfe5ceb741d1e793'  # Change this!
jwt = JWTManager(app)
...
SMTP_PORT = 465
SMTP_USERNAME = 'chatbotcustomerservice2025@gmail.com'
SMTP_PASSWORD = 'gizocbozpzdpvkts'
...
def verify_admin():
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT role FROM users WHERE email = :email", {'email': email})
    result = cursor.fetchone()
    if not result or result[0] != "admin":
        return jsonify({"message": "Unauthorized"}), 403
    return jsonify({"message": "AccessGranted"}), 200
...
def resend_code():
    ...
    try:
        ...
        You requested a resend of your password reset code.
        ...
        ChatBot Team
        """
        send_email(email, "Resent Password Reset Code", message)
